...
| `/api/tasks` | `tasks.py` | 任务 CRUD、执行、日志 |
| `/api/sign-tasks` | `sign_tasks_v2.py` | 签到任务管理、执行、历史 |
...
| 任务迁移 | 3 | 旧版 tasks → sign_tasks 迁移检查/执行/回滚 |

#### `batch.py` — 1 个端点（前缀 `/api/batch`）

- 批量操作：`enable` / `disable` / `delete` / `run`
- 逐任务处理 + 独立错误处理（单任务失败不影响其他）
- ⚠️ 使用旧版 ORM `tasks` 服务，与 `sign_tasks_v2.py` 文件存储体系不同
...
A: `tasks.py` + `batch.py` 使用旧版 SQLAlchemy ORM 存储；`sign_tasks_v2.py` 使用文件存储体系（`ConfigService`）。两套并存，新功能应使用 `sign_tasks_v2.py`。
